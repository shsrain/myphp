...
//--></script></head><body width="100%"><!--后台主页头部--><div id="loader" ><?php echo L('load_page');?></div><div id="result" class="result none"></div><div class="mainbox"><?php if(empty($_GET['isajax'])): ?><div id="nav" class="mainnav_title"><div id="lang"><?php if(APP_LANG): parse_str($_SERVER['QUERY_STRING'],$urlarr); unset($urlarr[l]); $url='index.php?'.http_build_query($urlarr); if(is_array($Lang)): $i = 0; $__LIST__ = $Lang;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$langvo): $mod = ($i % 2 );++$i;?><a href="<?php echo ($url); ?>&l=<?php echo ($langvo["mark"]); ?>" <?php if($langvo[mark]==$_SESSION['FCDREP_lang']): ?>class="on"<?php endif; ?>><?php echo ($langvo["name"]); ?></a><?php endforeach; endif; else: echo "" ;endif; endif; ?></div><ul><a href="<?php echo U($nav[bnav][model].'/'.$nav[bnav][action],$nav[bnav][data]);?>"><?php echo ($nav["bnav"]["name"]); ?></a>|
	...
	</script><?php endif; ?><!--管理主页框架--><!--栏目列表--><form name="myform" action="<?php echo U('Category/listorder');?>" method="post"><div class="pad-lr-10"><div class="table-list"><table width="100%" cellspacing="0"><thead><tr><th width="40"><?php echo L('listorder');?></th><th width="40">catid</th><th><?php echo L('catname');?></th><th width="80"><?php echo L('use_module');?></th><th width="40"><?php echo L('ismenu');?></th><th width="30"><?php echo L('fangwen');?></th><th width="220"><?php echo L('manage');?></th></tr></thead><tbody><?php echo ($categorys); ?></tbody></table><div class="btn"><input type="submit" class="button" name="dosubmit" value="<?php echo L('listorder');?>" /></div></div></div></div></form></div></body></html>