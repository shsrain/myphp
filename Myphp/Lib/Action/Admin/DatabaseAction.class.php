...
class DatabaseAction extends AdminbaseAction{
    ...
    {
		parent::_initialize();
		$this->datadir = __ROOT__.'Public/Data/';
	...
	public function docommand()
    {
		...
		if($do=='show'){
			 foreach ((array)$tables as $t){
				$result = $this->db->execute("SHOW COLUMNS FROM {$t}");
			}
			if(false != $result){ $this->success(L('do_ok'));}else{ $this->error($r['dbError']);}
		}else{
	...
	*/
	public function backup(){
		...
		$filename = !$filename ? 'Backup_'.rand_string(10).'_'.date('YmdH') : $filename;
