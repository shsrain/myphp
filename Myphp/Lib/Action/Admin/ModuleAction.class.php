...
class ModuleAction extends AdminbaseAction {
	...
	function _before_update(){
		...
		$r = $this->dao->find($_POST['id']);
		$moduletype = $r['type'];
		$_POST['forward'] = U(MODULE_NAME.'/index?type='.$moduletype);
	}
	...
	function insert(){	
		...
		if ($moduleid  !==false) {
			...
			$this->assign ( 'jumpUrl', U(MODULE_NAME.'/index?type='.$_POST['type']) );
	...
	function delete() {	
		...
		if($m){
			...
			$this->assign ( 'jumpUrl', U(MODULE_NAME.'/index?type='.$moduletype) );
