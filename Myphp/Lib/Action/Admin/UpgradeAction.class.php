...
class UpgradeAction extends AdminbaseAction {
	...
	private $_filearr = array('Myphp', 'Public', 'Core', '');
		...
		if(!empty($_GET['do'])) {
			...
			foreach($patchlist as $k=>$v) {		 
				...
				if($this->copyfileerror){
					...
					if(file_exists($upgradezip_source_path.'/myphp.sql')){
						$sqldata = file_get_contents($upgradezip_source_path.'/myphp.sql');
	...
	*/
	public function checkfile() {
			...
			$v=explode(' ',VERSION);
			$var=$v[0];
			$myphp_filemd5 = @file_get_contents($this->_url.$var.'/md5file/'.UPDATETIME.".php");
			$myphp_filemd5_arr = json_decode($myphp_filemd5, 1);unset($myphp_filemd5);

			$filemd5_arr_key = array_keys($this->filemd5_arr);
			$myphp_filemd5_arr_key = array_keys($myphp_filemd5_arr);

			$editfiles = array_diff($myphp_filemd5_arr, $this->filemd5_arr);
			$lostfile = array_diff( $myphp_filemd5_arr_key,$filemd5_arr_key);
			$unknowfile = array_diff($filemd5_arr_key, $myphp_filemd5_arr_key);
