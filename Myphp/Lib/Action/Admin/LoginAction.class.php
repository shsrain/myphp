...
class LoginAction extends Action{
    ...
    public function doLogin()
    {
		...
		if($lognum>=100)$this->error(L('login_error_count'));// 如果登录次数超过100次，禁止登录

        $condition = array();
        $condition['username'] =array('eq',$username);

		import ( '@.ORG.RBAC' );
		// 依据客户端提交的用户名执行一次thinkphp的find()放方法数据查询，查询返回值null：查询为空，false：查询错误，数据。
