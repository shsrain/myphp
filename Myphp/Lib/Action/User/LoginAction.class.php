...
class LoginAction extends BaseAction
{
	...
	function dologin()
	{
        ...
        if(empty($authInfo)) {
			...
			$myphp_auth = authcode($authInfo['id']."-".$authInfo['groupid']."-".$authInfo['password'], 'ENCODE', $myphp_auth_key);

			
 
			cookie('auth',$myphp_auth,$cookietime);
	...
	function sendmail(){
		...
		if($data){
			$myphp_auth = authcode($data['id']."-".$data['username']."-".$data['email'], 'ENCODE',$this->sysConfig['ADMIN_ACCESS'],3600*24*3);//3天有效期
			$username=$data['username'];
			$url =  'http://'.$_SERVER['HTTP_HOST'].U('User/Login/repassword?code='.$myphp_auth);
	...
	function emailcheck(){
		...
		if($_REQUEST['resend']){
			...
			if($this->member_config['member_emailcheck']){
						$myphp_auth = authcode($uid."-".$username."-".$email, 'ENCODE',$this->sysConfig['ADMIN_ACCESS'],3600*24*3);//3天有效期
						$url = 'http://'.$_SERVER['HTTP_HOST'].U('User/Login/regcheckemail?code='.$myphp_auth);
