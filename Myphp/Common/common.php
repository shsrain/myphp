...
function sendmail($tomail,$subject,$body,$config=''){
		...
		//$mail->From ="myphp@163.com";
		//$mail->FromName = "企业建站系统（CMS）";
		$mail->SetFrom($config['mail_from'], $config['site_name']);     //发送者邮箱
		$mail->AddAddress($tomail); //可同时发多个
		//$mail->AddReplyTo('147613338@qq.com', 'myphp'); //回复到这个邮箱
	...
	// 栏目缓存
		...
		if(APP_LANG){
			...
			foreach((array)$lang as $key=>$r){
				...
				if($langid){
					$lang = $key;
					$list = $Model->where('lang='.$langid)->order('listorder')->select ();// 根据语言id获取每个语言的栏目信息。
					$pkid = $Model->getPk ();
					$data = array ();
					
					// 将从数据库中读取的多语言栏目保存至栏目缓存文件中。
					foreach ( $list as $key => $val ) {
						$data [$val [$pkid]] = $val;
...
function geturl($cat,$data='',$Urlrule=''){
		...
		$module = $cat['module'];// 模型名称
		$moduleid =$cat['moduleid']; //模型ID
		$catid=$cat['id']; //分类ID
...
function  sql_split($sql,$tablepre) {
	...
	if($tablepre != "myphp_") $sql = str_replace("myphp_", $tablepre, $sql);
...
function get_site_url() {
	...
	$php_self = $_SERVER['PHP_SELF'] ? str_safe_replace($_SERVER['PHP_SELF']) : str_safe_replace($_SERVER['SCRIPT_NAME']);
	$path_info = isset($_SERVER['PATH_INFO']) ? str_safe_replace($_SERVER['PATH_INFO']) : '';
	$relate_url = isset($_SERVER['REQUEST_URI']) ? str_safe_replace($_SERVER['REQUEST_URI']) : $php_self.(isset($_SERVER['QUERY_STRING']) ? '?'.str_safe_replace($_SERVER['QUERY_STRING']) : $path_info);
	return $sys_protocal.(isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '').$relate_url;
}

/**
 * 获取当前页面的域名,包括协议类型,域名与端口号
 */
 ...
 * @return string
 */
function str_safe_replace($string) {
 ...
 * 判断客户端是否来自手机
 * 
 * return 如果是则返回true，电脑访问则返回false
 *
*/
