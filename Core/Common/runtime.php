...
function build_runtime_cache($append='') {
	...
	if(!defined('CMS_LICENSE')){ define('CMS_LICENSE',false); }// 关闭许可验证。
	if(!CMS_LICENSE){
		$content .= 'define(\'CMS_KEY\',true);';
	}else{
		/*授权验证开始*/
		
		// 如果编译文件中存在CMS_KEY常量定义，则删除定义常量，并退出程序运行。
		if(defined('CMS_KEY')){ $content = preg_replace('/defined\(\'CMS_KEY\'\) or define\(\'CMS_KEY\',\'(.+?)\'\)\;/','',$content);exit;}
		// 从服务器配置的主机名称中获取主域名并保存到$domain变量中。
		preg_match('/[\w][\w-]*\.(?:com\.cn|net\.cn|com|cn|co|net|org|gov|cc|biz|info)(\/|$)/isU', $_SERVER['SERVER_NAME'], $domain);
		// 将第一次匹配得到的主域名保存至$domain中。
		$domain = $domain[0];
		// 如果站点根目录存在以站点域名命名的php文件，则载入，并进行授权验证。
		if(is_file(__ROOT__.$domain.'.php')){	
			include __ROOT__.$domain.'.php';
					
			// 对使用 MIME base64 编码的$code数据进行解码
			// 使用authcode()方法进行解密
			// 使用eval函数把字符串按照PHP代码来计算
			eval(authcode(base64_decode($code)));
			// 定义CMS_KEY常量，并将授权验证结果（为boolean值）保存至CMS_KEY中，将CMS_KEY授权验证常量保存至$content预编译字符串中。
			$content .=  sha1( $domain.$key['key'])==$key['code']/*如果主域名加上$key['key']的字符串SHA-1散列与$key['code']相等*/ ? ' defined(\'CMS_KEY\') ?  exit : define(\'CMS_KEY\',true);'/*如果事先定义CMS_KEY常量，则退出程序执行，否则定义CMS_KEY为true，验证成功*/ : 'define(\'CMS_KEY\',false);'/*定义CMS_KEY为false，验证失败*/;
		}// 如果不存在站点域名命名的php文件的授权验证文件，则定义CMS_KEY授权验证常量值为false。标识验证失败。
			else{
			$content .= 'define(\'CMS_KEY\',false);';
		}
		/*授权验证结束*/
    }    
